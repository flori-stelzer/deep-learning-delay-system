...
void initialize_weights(mat &input_weights, cube &hidden_weights, mat &output_weights, int D, int L, int N, vector<int> diag_indices, 
						double initial_input_weigt_radius, double initial_hidden_weigt_radius, double initial_output_weigt_radius, bool save_to_file){
	/*
	...
	*/
	...
	input_weights = M_PI * eye(N, M + 1);
	for (int n = 0; n < N; ++n){
		input_weights(n, M) = - 0.5 * M_PI;
	}
...
void print_parameters(string results_file_name, string print_msg, string task, string system_simu, string grad_comp, bool gradient_check,
...
void print_results(string results_file_name, vector<int> diag_indices, vector<double> training_accuracy_vector, vector<double> accuracy_vector, vector<double> similarity_vector){
	/*
	...
	*/
	...
	for (int i = 0; i < diag_indices.size() - 1; ++i){
