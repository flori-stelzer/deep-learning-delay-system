...
void print_parameters(std::string results_file_name, std::string print_msg, std::string task, std::string system_simu, std::string grad_comp, bool gradient_check, int N, int L, int D, double theta, double alpha, std::string diag_method, int diag_distance, std::string diag_file_path, int number_of_epochs, double eta_0, double eta_1, bool pixel_shift, bool input_noise, double training_noise_sigma, int N_h, double exp_precision);
void print_results(std::string results_file_name, std::vector<int> diag_indices, std::vector<double> training_accuracy_vector, std::vector<double> accuracy_vector, std::vector<double> similarity_vector);
