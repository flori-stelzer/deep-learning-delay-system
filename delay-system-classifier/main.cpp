...
int main(int argc, char const *argv[])
{
	...
	bool no_input_layer = katana::getCmdOption_bool(argv, argv + argc, "-no_input_layer", false);  // only for CIFAR-10
	// dropout
	...
	if (save_init_weights){
		...
		initialize_weights(input_weights, hidden_weights, output_weights, D, L, N, diag_indices,
						   initial_input_weigt_radius, initial_hidden_weigt_radius, initial_output_weigt_radius, true, no_input_layer);	
	...
	// The test are only used if the precompiler flag TESTING is set
	...
	for (int validation_batch_index = validation_batch_start_index; validation_batch_index < validation_batch_end_index + 1; ++validation_batch_index){
		...
		vector<int> diag_indices;
		diag_indices = get_diag_indices(N, D, diag_method, diag_distance, diag_file_path, validation_batch);
		...
		if (init_weights_from_file){
			...
			initialize_weights(input_weights, hidden_weights, output_weights, D, L, N, diag_indices,
						   initial_input_weigt_radius, initial_hidden_weigt_radius, initial_output_weigt_radius, false, no_input_layer);
		}
		...
		for (int epoch = 0; epoch < number_of_epochs; ++epoch){
			...
			// make vector with randomly shuffled indices between 0 and 49999 (for MNIST, Fashion-MNIST or CIFAR-10; different number for SVHN) for each epoch of the stochastic gradient descent 
			vector<int> index_vector;
			uint actually_used_number_of_training_batches = (number_of_training_batches - 1);
#ifdef TESTING
			actually_used_number_of_training_batches = number_of_training_batches;
#endif
			for (uint index = 0; index < actually_used_number_of_training_batches*training_batch_size; ++index){
			...
			for (int index : index_vector){
				...
				// input layer
				if (!no_input_layer)  //so if there IS an input layer... we can mask it, otherwise it would maybe mess up stuff?
				{				
					for (int m = 0; m < M; ++m){
...
// 				}
			...
			training_accuracy_vector.push_back(double(correct_count) / (double)(actually_used_number_of_training_batches*training_batch_size/100.0));


			// loop for validation:
			correct_count = 0;
#ifdef TESTING
			for (uint index = 0; index < test_batch_size; ++index){

				vec input_data = test_images.row(index).t();
				int label = test_labels[index];
#else //Validation
			for (uint index = 0; index < training_batch_size; ++index){
			...
			cout << "epoch " << epoch + 1 << ": testing accuracy = " << double(correct_count) / (double)(test_batch_size/100.0) << endl;
#else //Validation
