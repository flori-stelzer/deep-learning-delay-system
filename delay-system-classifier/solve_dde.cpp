...
void solve_dde_heun(mat &activations, mat &node_states, double (&output_activations)[P], double (&outputs)[P], vec &g_primes,
				   ...
				   int N, int L, int N_h, bool no_input_layer)
{
	/*
	...
	*/
	...
	{
		for (int n = 0; n < N; ++n){
			activations(0, n) = input_data(n); //if I want to make a simple trainable layer, this will have to be changed again. This is also ignoring dropouts
			g_primes(n) = 0;
		}
	}
	else
...
void solve_dde_ibp(mat &activations, mat &node_states, double (&output_activations)[P], double (&outputs)[P], vec &g_primes,
				   vec &input_data, mat input_weights, cube hidden_weights, mat output_weights, vector<int> &diag_indices, double theta, double alpha,
				   int N, int L, int N_h, int record_example, int rec_step, bool no_input_layer)
{
	/*
	...
	*/
	...
	{
		for (int n = 0; n < N; ++n){
			activations(0, n) = input_data(n); //if I want to make a simple trainable layer, this will have to be changed again.
			g_primes(n) = 0;
		}
	}
	else
